...
public class PIDShoulderToPosition extends Command {
...
//    private final static int I_ZONE_IN_REVOLUTIONS = 50; //100;
    ...
    public PIDShoulderToPosition(double position) {
...
//      Robot.resetNavXAngle();
        Robot.lift.enablePositionControl(Robot.lift.shoulderPrimary);
        
        Robot.lift.shoulderPrimary.config_kP(pidIdx, specificDistanceP, timeoutMs);
        
        Robot.lift.shoulderPrimary.config_kI(pidIdx, specificDistanceI, timeoutMs);
        
        Robot.lift.shoulderPrimary.config_kD(pidIdx, specificDistanceD, timeoutMs);
        
        Robot.lift.shoulderPrimary.config_kF(pidIdx, specificDistanceF, timeoutMs);
        
//        Robot.DRIVE_SUBSYSTEM.talonDriveLeftPrimary.ClearIaccum();
//        Robot.DRIVE_SUBSYSTEM.talonDriveRightPrimary.ClearIaccum();
        
        Robot.lift.shoulderPrimary.configNominalOutputForward(+MIN_PERCENT_VOLTAGE, timeoutMs);
        Robot.lift.shoulderPrimary.configNominalOutputReverse(-MIN_PERCENT_VOLTAGE, timeoutMs);
        Robot.lift.shoulderPrimary.configPeakOutputForward(+MAX_PERCENT_VOLTAGE, timeoutMs);
        Robot.lift.shoulderPrimary.configPeakOutputReverse(-MAX_PERCENT_VOLTAGE, timeoutMs);
...
//        Robot.DRIVE_SUBSYSTEM.talonDriveRightPrimary.setCloseLoopRampRate(rampRate);
        ...
        Robot.lift.shoulderPrimary.configAllowableClosedloopError(slotIdx, allowableCloseLoopError, timeoutMs);
...
//        Robot.DRIVE_SUBSYSTEM.talonDriveRightPrimary.config_IntegralZone(slotIdx, I_ZONE_IN_REVOLUTIONS, timeoutMs);
            ...
            Robot.lift.shoulderPrimary.configMotionCruiseVelocity(10000, timeoutMs); //7500, 20500, 7500, 20000
            Robot.lift.shoulderPrimary.configMotionAcceleration(20000, timeoutMs); //test 5000


        Robot.lift.setPID(driveTicks, driveTicks, Robot.lift.shoulderPrimary);
    }
...
//        SmartDashboard.putNumber("Straight", OI.latestDistanceDriven);
        Robot.lift.enableVBusControl(Robot.lift.shoulderPrimary);
//      Robot.resetNavXAngle();
        Robot.lift.stop(Robot.lift.shoulderPrimary);
    }
