...
public class PIDWristToPosition extends Command {
...
//    private final static int I_ZONE_IN_REVOLUTIONS = 50; //100;
    ...
    public PIDWristToPosition(double position) {
...
//      Robot.resetNavXAngle();
        Robot.lift.enablePositionControl(Robot.lift.wristPrimary);
        
        Robot.lift.wristPrimary.config_kP(pidIdx, specificDistanceP, timeoutMs);
        
        Robot.lift.wristPrimary.config_kI(pidIdx, specificDistanceI, timeoutMs);
        
        Robot.lift.wristPrimary.config_kD(pidIdx, specificDistanceD, timeoutMs);
        
        Robot.lift.wristPrimary.config_kF(pidIdx, specificDistanceF, timeoutMs);
        
//        Robot.DRIVE_SUBSYSTEM.talonDriveLeftPrimary.ClearIaccum();
//        Robot.DRIVE_SUBSYSTEM.talonDriveRightPrimary.ClearIaccum();
        
        Robot.lift.wristPrimary.configNominalOutputForward(+MIN_PERCENT_VOLTAGE, timeoutMs);
        Robot.lift.wristPrimary.configNominalOutputReverse(-MIN_PERCENT_VOLTAGE, timeoutMs);
        Robot.lift.wristPrimary.configPeakOutputForward(+MAX_PERCENT_VOLTAGE, timeoutMs);
        Robot.lift.wristPrimary.configPeakOutputReverse(-MAX_PERCENT_VOLTAGE, timeoutMs);
...
//        Robot.DRIVE_SUBSYSTEM.talonDriveRightPrimary.setCloseLoopRampRate(rampRate);
        ...
        Robot.lift.wristPrimary.configAllowableClosedloopError(slotIdx, allowableCloseLoopError, timeoutMs);
...
//        Robot.DRIVE_SUBSYSTEM.talonDriveRightPrimary.config_IntegralZone(slotIdx, I_ZONE_IN_REVOLUTIONS, timeoutMs);
            ...
            Robot.lift.wristPrimary.configMotionCruiseVelocity(10000, timeoutMs); //7500, 20500, 7500, 20000
            Robot.lift.wristPrimary.configMotionAcceleration(20000, timeoutMs); //test 5000


        Robot.lift.setPID(driveTicks, driveTicks, Robot.lift.wristPrimary);
    }
...
//        SmartDashboard.putNumber("Straight", OI.latestDistanceDriven);
        Robot.lift.enableVBusControl(Robot.lift.wristPrimary);
//      Robot.resetNavXAngle();
        Robot.lift.stop(Robot.lift.wristPrimary);
    }
