...
public class PIDDriveInchesRight extends Command {
  ...
  public PIDDriveInchesRight(double inches) {
    inch_goal = inches;
  ...
  protected void execute() {
    Robot.DRIVE_SUBSYSTEM.setPIDRight(rightGoal);
    ...
    SmartDashboard.putNumber("goal to travel to right", rightGoal);
  }
...
// Robot.DRIVE_SUBSYSTEM.getLeftTicks() >  left_end_ticks - stop_threshold_ticks && Robot.DRIVE_SUBSYSTEM.getLeftTicks() < left_end_ticks + stop_threshold_ticks && Robot.DRIVE_SUBSYSTEM.getRightTicks() > right_end_ticks - stop_threshold_ticks && Robot.DRIVE_SUBSYSTEM.getRightTicks() < right_end_ticks + stop_threshold_ticks
    ...
    if((Robot.DRIVE_SUBSYSTEM.getRightRevs() > rightGoal - stop_threshold_revs) && (Robot.DRIVE_SUBSYSTEM.getRightRevs() < rightGoal + stop_threshold_revs)) { //&& Robot.DRIVE_SUBSYSTEM.getRightTicks() > 100) {
      onTargetCount++;
    } else {
      onTargetCount = 0;
    }
